import { execFile } from 'node:child_process';
import fs from 'node:fs/promises';
import os from 'node:os';
import path from 'node:path';
import { promisify } from 'node:util';
import { afterEach, beforeEach, expect, test } from 'vitest';
import { getToolDefinitions, getToolHandler } from '../../src/mcp/tools.js';
import type { ProveReport } from '../../src/types.js';

const execFileAsync = promisify(execFile);

let tmp: string;

beforeEach(async () => {
  tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'projscan-mcp-prove-'));
  await fs.writeFile(
    path.join(tmp, 'package.json'),
    JSON.stringify({
      name: 'fixture',
      version: '1.0.0',
      type: 'module',
      scripts: { test: 'vitest run' },
      devDependencies: { vitest: '^3.0.0' },
    }),
  );
  await fs.writeFile(path.join(tmp, '.gitignore'), '.env\n.projscan-memory/\n');
  await fs.mkdir(path.join(tmp, 'src/core'), { recursive: true });
  await fs.mkdir(path.join(tmp, 'tests/core'), { recursive: true });
  await fs.writeFile(
    path.join(tmp, 'src/core/bugHunt.ts'),
    "export function buildBugHuntReport(findings: string[]): string[] { return findings; }\n",
  );
  await fs.writeFile(
    path.join(tmp, 'tests/core/bugHunt.test.ts'),
    "test('builds report', () => undefined);\n",
  );
  await git(['init']);
  await git(['config', 'user.email', 'projscan@example.test']);
  await git(['config', 'user.name', 'Projscan Test']);
  await git(['add', '.']);
  await git(['commit', '-m', 'initial']);
});

afterEach(async () => {
  await fs.rm(tmp, { recursive: true, force: true });
});

test('lists projscan_prove MCP tool with contract inputs', () => {
  const tool = getToolDefinitions().find((entry) => entry.name === 'projscan_prove');

  expect(tool).toBeDefined();
  expect(tool?.description).toContain('Proof Contract');
  expect(tool?.description).toContain('Proof Receipt');
  expect(tool?.inputSchema.properties?.intent.description).toContain('change intent');
  expect(tool?.inputSchema.properties?.changed.description).toContain('working tree');
  expect(tool?.inputSchema.properties?.contract_path.description).toContain('contract');
  expect(tool?.inputSchema.properties?.record_command.description).toContain('Record');
  expect(tool?.inputSchema.properties?.exit_code.description).toContain('Exit code');
});

test('projscan_prove returns an intent Proof Contract', async () => {
  const handler = getToolHandler('projscan_prove');
  expect(handler).toBeDefined();

  const result = (await handler?.(
    {
      intent: 'split bugHunt.ts into ranking, evidence, and output modules',
      max_files: 3,
      save_contract_path: '.projscan/proof-contract.json',
    },
    tmp,
  )) as { prove: ProveReport };

  expect(result.prove.schemaVersion).toBe(1);
  expect(result.prove.mode).toBe('intent');
  expect(result.prove.contract?.allowedFiles).toContain('src/core/bugHunt.ts');
  expect(result.prove.contract?.proofCommands).toContain(
    'projscan assess --mode fix-first --format json',
  );
});

test('projscan_prove records and replays proof ledger evidence', async () => {
  const handler = getToolHandler('projscan_prove');
  const contractResult = (await handler?.(
    {
      intent: 'split bugHunt.ts into ranking, evidence, and output modules',
      max_files: 3,
      save_contract_path: '.projscan/proof-contract.json',
    },
    tmp,
  )) as { prove: ProveReport };
  const command = contractResult.prove.contract?.proofCommands[0];
  expect(command).toBeDefined();

  await fs.writeFile(
    path.join(tmp, 'src/core/bugHunt.ts'),
    "export function buildBugHuntReport(findings: string[]): string[] { return findings.map(String); }\n",
  );
  const recordResult = (await handler?.(
    {
      record_command: command,
      exit_code: 0,
      duration_ms: 123,
      summary: 'passed with token=secret-value',
      log_path: '.projscan/proof-logs/mcp.log',
    },
    tmp,
  )) as { prove: ProveReport };

  expect(recordResult.prove.mode).toBe('record');
  expect(recordResult.prove.ledgerRecord?.outputSummary).toContain('token=[redacted]');

  const changedResult = (await handler?.(
    {
      changed: true,
      contract_path: '.projscan/proof-contract.json',
    },
    tmp,
  )) as { prove: ProveReport };

  expect(changedResult.prove.receipt?.proofStatus.commandsRun).toContain(command);
});

async function git(args: string[]): Promise<void> {
  await execFileAsync('git', args, { cwd: tmp });
}
