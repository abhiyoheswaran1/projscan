...
beforeEach(async () => {
  tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'projscan-cli-proof-broker-'));
  await fs.writeFile(
...
test('proof-broker renders JSON, PR-comment markdown, and console handoff output', async () => {
  ...
  expect(report.requiredProof.map((row: { id: string }) => row.id)).toContain(
  ...
  expect(markdown.stdout).toContain('## Projscan PR Passport');
  expect(markdown.stdout).toContain('Reviewer action');
  ...
  expect(consoleResult.stdout).toContain('Projscan Proof Broker');
  expect(consoleResult.stdout).toContain('Required Proof');
