...
- **641 tests passing** (+43 over 0.10). New coverage: AST CC for JS/TS + Python, Python CC node types (incl. comprehensions and match/case), coupling fan-in/out/instability + Tarjan SCC (DAG, 2-cycle, 3-cycle, disjoint, self-loop ignored), hotspot LOC→CC switch + LOC fallback regression, monorepo detection (npm/yarn/pnpm/Nx fallback), prefix-matching against `packages/a` vs `packages/ab`, telemetry config resolution + env override + JSONL append + sink-failure silence, Go parser/imports/exports/CC + end-to-end fixture (example.com/widget module with main + internal/util), pack-smoke test extended for tree-sitter-go.wasm.
