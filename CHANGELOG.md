...
- Added `projscan start --save-mission <dir>`, which writes a Mission Control handoff bundle with a quickstart README, next-step command and MCP-call files, Markdown, JSON, proof commands, and a manifest.
...
- `missionControl.executionPlan` proof steps and `missionControl.resume.checklist` `run_proof` rows now include MCP `tool` / `args` metadata when a proof command is MCP-callable.
