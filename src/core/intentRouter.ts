...
function routeKeywordMatches(entry: RouteEntry, keyword: string, tokens: Set<string>, hasFilePath: boolean, hasPackageRemoval: boolean, hasPackageChange: boolean, hasEnvVar: boolean, hasQuotedText: boolean): boolean {
  if (!tokens.has(keyword)) return false;
  if (entry.tool === 'projscan_privacy_check' && searchIntegrationContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_privacy_check' && searchUiInteractionContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_privacy_check' && !privacyCheckKeywordMatches(keyword, tokens)) return false;
  if (entry.tool === 'projscan_understand' && !understandKeywordMatches(keyword, tokens)) return false;
  if (entry.tool === 'projscan_understand' && searchEnvLookupContextMatches(tokens, hasEnvVar)) return false;
  if (entry.tool === 'projscan_understand' && searchQuotedDebugTextContextMatches(tokens, hasQuotedText)) return false;
  if (entry.tool === 'projscan_understand' && searchTestDataContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_understand' && searchDataContractContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_understand' && searchUiInteractionContextMatches(tokens)) return false;
  ...
  if (entry.tool === 'projscan_understand' && searchInfraArtifactContextMatches(tokens) && !localServiceSetupCommandContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_understand' && searchDomainWorkflowContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_understand' && searchCommunicationArtifactContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_understand' && searchStateManagementContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_understand' && searchDataAccessContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_understand' && searchNavigationLayoutContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_understand' && searchFrontendPageRouteContextMatches(tokens)) return false;
  ...
  if (entry.tool === 'projscan_explain_issue' && styleSystemFailureContextMatches(tokens)) return false;
  ...
  if (entry.tool === 'projscan_dataflow' && domainWorkflowPlanningContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_dataflow' && stateManagementPlanningContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_dataflow' && dataAccessPlanningContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_dataflow' && ['process', 'processes'].includes(keyword) && searchEnvLookupContextMatches(tokens, hasEnvVar)) return false;
  if (entry.tool === 'projscan_dataflow' && searchQuotedDebugTextContextMatches(tokens, hasQuotedText) && !explicitDataflowContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_dataflow' && searchBackgroundWorkContextMatches(tokens) && !explicitDataflowContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_dataflow' && searchObservabilityContextMatches(tokens) && !explicitDataflowContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_dataflow' && searchTestDataContextMatches(tokens) && !explicitDataflowContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_dataflow' && searchReliabilityContextMatches(tokens) && !explicitDataflowRiskContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_dataflow' && searchDataContractContextMatches(tokens) && !explicitDataflowRiskContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_dataflow' && searchUiInteractionContextMatches(tokens) && !explicitDataflowRiskContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_dataflow' && searchIntegrationContextMatches(tokens) && !explicitDataflowRiskContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_dataflow' && searchApiContractContextMatches(tokens) && !explicitDataflowRiskContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_dataflow' && searchInfraArtifactContextMatches(tokens) && !explicitDataflowRiskContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_dataflow' && searchDomainWorkflowContextMatches(tokens) && !explicitDataflowRiskContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_dataflow' && searchCommunicationArtifactContextMatches(tokens) && !explicitDataflowRiskContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_dataflow' && searchStateManagementContextMatches(tokens) && !explicitDataflowRiskContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_dataflow' && searchDataAccessContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_dataflow' && searchNavigationLayoutContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_dataflow' && searchFrontendPageRouteContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_dataflow' && searchStyleSystemContextMatches(tokens)) return false;
  if (hasFilePath && keyword === 'start' && ['projscan_hotspots', 'projscan_start'].includes(entry.tool)) return false;
  if (entry.tool === 'projscan_file' && keyword === 'read') return hasFilePath;
  if (entry.tool === 'projscan_file' && ['review', 'reviewer', 'reviewers'].includes(keyword)) return hasFilePath;
  if (entry.tool === 'projscan_file' && keyword === 'owns') return hasFilePath;
  if (entry.tool === 'projscan_file' && ['risk', 'risks', 'risky', 'dangerous'].includes(keyword)) return hasFilePath;
  ...
  if (entry.tool === 'projscan_file' && keyword === 'file' && searchConfigLookupContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_file' && keyword === 'file' && searchToolingConfigContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_hotspots' && searchUiInteractionContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_hotspots' && searchNavigationLayoutContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_hotspots' && searchFrontendPageRouteContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_hotspots' && searchStyleSystemContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_hotspots' && ['where', 'start'].includes(keyword) && !hotspotWhereContextMatches(tokens, hasFilePath)) return false;
  if (entry.tool === 'projscan_impact' && ['delete', 'remove'].includes(keyword) && !hasFilePath && !impactDeleteContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_impact' && hasEnvVar && ['depends', 'affect', 'callers', 'used', 'usage', 'referenced', 'called', 'api', 'apis'].includes(keyword)) return false;
  if (entry.tool === 'projscan_impact' && ['used', 'usage', 'referenced', 'called'].includes(keyword) && searchTestDataContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_impact' && ['used', 'usage', 'referenced', 'called'].includes(keyword) && searchReliabilityContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_impact' && ['drop', 'schema', 'table', 'column', 'database', 'db', 'migration', 'migrations'].includes(keyword) && searchDataContractContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_impact' && ['drop', 'schema', 'table', 'column', 'database', 'db', 'migration', 'migrations'].includes(keyword) && !impactDatabaseContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_impact' && impactApiKeywordMatches(keyword) && !impactApiContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_impact' && ['revert', 'rollback', 'undo', 'backout', 'back', 'out', 'recover'].includes(keyword) && !impactRollbackContextMatches(keyword, tokens)) return false;
  if (entry.tool === 'projscan_semantic_graph' && ['uses', 'depend', 'depends', 'installed'].includes(keyword) && !packageDependencyLookupContextMatches(tokens, hasFilePath)) return false;
  if (entry.tool === 'projscan_semantic_graph' && ['defined', 'definition'].includes(keyword) && searchBackgroundWorkContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_semantic_graph' && ['defined', 'definition'].includes(keyword) && searchTestDataContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_semantic_graph' && ['defined', 'definition'].includes(keyword) && searchAuthorizationContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_semantic_graph' && ['defined', 'definition'].includes(keyword) && searchDataContractContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_semantic_graph' && ['defined', 'definition'].includes(keyword) && searchDataAccessContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_semantic_graph' && ['defined', 'definition'].includes(keyword) && searchStyleSystemContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_doctor' && ['safe', 'safely', 'delete', 'remove'].includes(keyword) && !doctorCleanupDeleteContextMatches(tokens, hasFilePath, hasPackageRemoval)) return false;
  if (entry.tool === 'projscan_evidence_pack' && !evidencePackKeywordMatches(keyword, tokens)) return false;
  if (entry.tool === 'projscan_search' && ['search', 'find', 'locate', 'where', 'show', 'code'].includes(keyword) && doctorCleanupDiscoveryContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['test', 'tests', 'spec', 'specs', 'cover', 'covers', 'covering'].includes(keyword) && !searchTestLocationContextMatches(tokens, hasFilePath)) return false;
  if (entry.tool === 'projscan_search' && ['feature', 'features', 'flag', 'flags'].includes(keyword) && !searchFeatureFlagContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['env', 'var', 'vars', 'variable', 'variables', 'process', 'used', 'controls', 'control'].includes(keyword) && !searchEnvLookupContextMatches(tokens, hasEnvVar) && !searchTestDataContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['error', 'errors', 'message', 'messages', 'throw', 'throws', 'thrown', 'log', 'logs', 'logged', 'logging'].includes(keyword) && !searchQuotedDebugTextContextMatches(tokens, hasQuotedText) && !searchObservabilityContextMatches(tokens) && !searchUiInteractionContextMatches(tokens) && !searchDomainWorkflowContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && keyword === 'check' && !searchObservabilityContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['metric', 'metrics', 'prometheus', 'analytics', 'event', 'events', 'alert', 'alerts', 'sentry', 'datadog', 'dashboard', 'dashboards', 'emit', 'emits', 'emitted', 'send', 'sends', 'initialize', 'initialise', 'init'].includes(keyword) && !searchObservabilityContextMatches(tokens) && !searchIntegrationContextMatches(tokens) && !searchCommunicationArtifactContextMatches(tokens) && !searchNavigationLayoutContextMatches(tokens) && !searchFrontendPageRouteContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['input', 'validation', 'validate', 'validates', 'validator', 'schema', 'schemas', 'zod', 'params', 'param', 'query', 'queries', 'parsed', 'json', 'serialize', 'serializes', 'serialization', 'response', 'format', 'formats', 'formatting', 'date', 'transaction', 'transactions', 'wrap', 'wraps', 'started', 'database', 'db', 'lock', 'locks', 'locking', 'row', 'optimistic', 'unique', 'uniqueness', 'enforced', 'email', 'pagination', 'cursor', 'cursors', 'builds'].includes(keyword) && !searchDataContractContextMatches(tokens) && !searchIntegrationContextMatches(tokens) && !searchApiContractContextMatches(tokens) && !searchCommunicationArtifactContextMatches(tokens) && !searchStateManagementContextMatches(tokens) && !searchDataAccessContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['openapi', 'swagger', 'trpc', 'grpc', 'protobuf', 'proto', 'protos', 'router', 'routers', 'resolver', 'resolvers'].includes(keyword) && !searchApiContractContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['docker', 'dockerfile', 'compose', 'containerfile', 'kubernetes', 'k8s', 'manifest', 'manifests', 'helm', 'chart', 'charts', 'terraform', 'tf', 'module', 'modules', 'cloudformation', 'cdk', 'pulumi', 'vercel', 'netlify', 'railway', 'fly', 'workflow', 'workflows', 'deploy', 'deploys', 'deployment', 'staging', 'production'].includes(keyword) && !searchInfraArtifactContextMatches(tokens) && !searchStyleSystemContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['password', 'reset', 'invite', 'invites', 'onboarding', 'flow', 'flows', 'csv', 'export', 'exports', 'audit', 'entries', 'refund', 'handling', 'payments', 'subscription', 'renewal', 'users'].includes(keyword) && !searchDomainWorkflowContextMatches(tokens) && !searchDataAccessContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['welcome', 'template', 'templates', 'copy', 'push', 'sms', 'verification', 'receipt', 'invoice', 'pdf'].includes(keyword) && !searchCommunicationArtifactContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['store', 'stores', 'stored', 'redux', 'slice', 'slices', 'selector', 'selectors', 'zustand', 'jotai', 'recoil', 'context', 'provider', 'providers', 'supplies', 'supplied', 'provides', 'provided', 'hook', 'hooks', 'react', 'mutation', 'mutations', 'fetches', 'fetched', 'invoices'].includes(keyword) && !searchStateManagementContextMatches(tokens) && !searchDataAccessContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['prisma', 'drizzle', 'typeorm', 'sequelize', 'sql', 'model', 'models', 'entity', 'entities', 'repository', 'repositories', 'dao', 'daos', 'saves', 'save', 'persist', 'persists', 'orders'].includes(keyword) && !searchDataAccessContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['sidebar', 'nav', 'navigation', 'menu', 'item', 'items', 'breadcrumb', 'breadcrumbs', 'layout', 'next', 'js', 'title', 'metadata', 'meta', 'billing', 'settings', 'checkout'].includes(keyword) && !searchNavigationLayoutContextMatches(tokens) && !searchFrontendPageRouteContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['segment', 'segments', 'not', 'found', '404'].includes(keyword) && !searchFrontendPageRouteContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['design', 'token', 'tokens', 'tailwind', 'theme', 'themes', 'css', 'global', 'imported', 'style', 'styles', 'styled', 'class', 'classes', 'dark', 'mode', 'breakpoint', 'breakpoints', 'color', 'colors'].includes(keyword) && !searchStyleSystemContextMatches(tokens) && !searchStateManagementContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['form', 'forms', 'submit', 'submits', 'submitted', 'loading', 'state', 'empty', 'results', 'boundary', 'toast', 'notification', 'notifications', 'success', 'keyboard', 'shortcut', 'shortcuts', 'command', 'palette', 'action', 'actions', 'modal', 'opened', 'component', 'page', 'i18n', 'translation', 'translations', 'aria', 'label', 'button', 'buttons', 'focus', 'trap'].includes(keyword) && !searchUiInteractionContextMatches(tokens) && !searchIntegrationContextMatches(tokens) && !searchCommunicationArtifactContextMatches(tokens) && !searchStateManagementContextMatches(tokens) && !searchNavigationLayoutContextMatches(tokens) && !searchFrontendPageRouteContextMatches(tokens) && !searchStyleSystemContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['call', 'calls', 'called', 'client', 'clients', 'sdk', 'sdks', 'integration', 'integrations', 'stripe', 'sendgrid', 's3', 'github', 'graphql', 'websocket', 'websockets', 'socket', 'sockets', 'connection', 'connections', 'rest', 'http', 'fetch', 'axios', 'external', 'service', 'services', 'upload', 'uploads', 'uploaded', 'sent', 'opened'].includes(keyword) && !searchIntegrationContextMatches(tokens) && !searchApiContractContextMatches(tokens) && !searchInfraArtifactContextMatches(tokens) && !searchStateManagementContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['rate', 'rates', 'limit', 'limits', 'limiting', 'throttle', 'throttling', 'cache', 'caches', 'cached', 'redis', 'invalidate', 'invalidates', 'invalidated', 'invalidation', 'retry', 'retries', 'retried', 'backoff', 'timeout', 'timeouts', 'request', 'requests', 'failed', 'set', 'sets', 'circuit', 'breaker', 'idempotency', 'idempotent', 'key', 'keys', 'signature', 'signatures', 'verified', 'verify', 'verification', 'debounce', 'debounced', 'protect', 'protects'].includes(keyword) && !searchReliabilityContextMatches(tokens) && !searchDataContractContextMatches(tokens) && !searchCommunicationArtifactContextMatches(tokens) && !searchNavigationLayoutContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['permission', 'permissions', 'checked', 'role', 'roles', 'access', 'admin', 'guard', 'guards', 'authorization', 'authorize', 'authorized', 'policy', 'policies', 'rbac', 'require', 'requires', 'required', 'login'].includes(keyword) && !searchAuthorizationContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['seed', 'seeds', 'data', 'fixture', 'fixtures', 'mock', 'mocks', 'factory', 'factories', 'storybook', 'story', 'stories', 'render', 'renders', 'rendered'].includes(keyword) && !searchTestDataContextMatches(tokens) && !searchUiInteractionContextMatches(tokens) && !searchNavigationLayoutContextMatches(tokens) && !searchFrontendPageRouteContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['background', 'job', 'jobs', 'cron', 'scheduled', 'schedule', 'scheduler', 'schedulers', 'worker', 'workers', 'queue', 'queues', 'processor', 'processors', 'task', 'tasks', 'defined', 'processes'].includes(keyword) && !searchBackgroundWorkContextMatches(tokens) && !searchTestDataContextMatches(tokens) && !searchAuthorizationContextMatches(tokens) && !searchApiContractContextMatches(tokens) && !searchStyleSystemContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['config', 'configuration', 'alias', 'aliases', 'define', 'defines'].includes(keyword) && !searchConfigLookupContextMatches(tokens) && !searchToolingConfigContextMatches(tokens) && !searchApiContractContextMatches(tokens) && !searchInfraArtifactContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['tsconfig', 'typescript', 'vite', 'vitest', 'jest', 'babel', 'webpack', 'pnpm', 'yarn', 'npm', 'package', 'manager', 'workspace', 'workspaces', 'lockfile', 'lockfiles', 'path', 'paths'].includes(keyword) && !searchToolingConfigContextMatches(tokens) && !searchConfigLookupContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['api', 'apis', 'route', 'routes', 'endpoint', 'endpoints'].includes(keyword) && !searchRouteHandlerContextMatches(tokens) && !searchAuthorizationContextMatches(tokens) && !searchDataContractContextMatches(tokens) && !searchIntegrationContextMatches(tokens) && !searchApiContractContextMatches(tokens) && !searchNavigationLayoutContextMatches(tokens) && !searchFrontendPageRouteContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['migration', 'migrations', 'generated', 'exist', 'exists', 'ran', 'show', 'file', 'files'].includes(keyword) && !searchMigrationLookupContextMatches(tokens) && !searchGeneratedContextMatches(tokens) && !searchConfigLookupContextMatches(tokens) && !searchToolingConfigContextMatches(tokens) && !searchBackgroundWorkContextMatches(tokens) && !searchTestDataContextMatches(tokens) && !searchDataContractContextMatches(tokens) && !searchCommunicationArtifactContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['code', 'handles', 'handled', 'handler', 'contains', 'logic', 'implemented', 'configured', 'created', 'creates', 'loaded', 'loader', 'parse', 'parses', 'parsed', 'middleware'].includes(keyword) && !searchCodeLocationContextMatches(tokens) && !searchGeneratedContextMatches(tokens) && !searchRouteHandlerContextMatches(tokens) && !searchReliabilityContextMatches(tokens) && !searchDataContractContextMatches(tokens) && !searchUiInteractionContextMatches(tokens) && !searchIntegrationContextMatches(tokens) && !searchApiContractContextMatches(tokens) && !searchDomainWorkflowContextMatches(tokens) && !searchDataAccessContextMatches(tokens) && !searchFrontendPageRouteContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['owner', 'owners', 'ownership', 'owns', 'team', 'area', 'ask', 'help', 'knows', 'expert', 'experts', 'contact', 'contacts'].includes(keyword) && !searchOwnershipContextMatches(tokens, hasFilePath) && !searchDomainWorkflowContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_search' && ['doc', 'docs', 'document', 'documentation', 'documented', 'readme', 'examples', 'example', 'guide'].includes(keyword) && !searchDocumentationContextMatches(tokens) && !searchApiContractContextMatches(tokens)) return false;
  ...
  if (entry.tool === 'projscan_dependencies' && !dependenciesKeywordMatches(keyword, tokens)) return false;
  if (['projscan_audit', 'projscan_outdated', 'projscan_upgrade'].includes(entry.tool) && ['dependency', 'dependencies', 'package', 'packages'].includes(keyword) && dependencyCycleContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_audit' && !auditKeywordMatches(keyword, tokens)) return false;
  if (entry.tool === 'projscan_workspaces' && searchToolingConfigContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_workspaces' && !workspacesKeywordMatches(keyword, tokens)) return false;
  if (entry.tool === 'projscan_upgrade' && keyword === 'update' && !hasPackageChange) return false;
  if (entry.tool === 'projscan_upgrade' && keyword === 'package' && packageImporterContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_upgrade' && ['package', 'dependency', 'dependencies', 'npm'].includes(keyword) && regressionLocalSetupContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_upgrade' && ['remove', 'drop', 'uninstall'].includes(keyword) && !hasPackageRemoval) return false;
  if (entry.tool === 'projscan_outdated' && keyword === 'npm' && !outdatedNpmContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_preflight' && keyword === 'ready' && !preflightReadyContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_preflight' && ['risk', 'risks'].includes(keyword) && !preflightRiskContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_preflight' && ['rebase', 'rebasing', 'conflict', 'conflicts', 'resolve', 'resolving', 'wrong', 'stuck'].includes(keyword) && !preflightBranchRecoveryContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_hotspots' && ['files', 'file', 'touch'].includes(keyword) && !hotspotFileRiskContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_hotspots' && ['performance', 'perf', 'bottleneck', 'bottlenecks', 'optimize', 'optimise', 'faster', 'slow'].includes(keyword) && !hotspotPerformanceContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_coordinate' && keyword === 'agent' && !coordinateAgentContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_coordinate' && ['who', 'else', 'working'].includes(keyword) && !coordinateWorkingContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_coordinate' && keyword === 'editing' && !coordinateWorkingContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_coordinate' && keyword === 'active' && !coordinateActiveContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_coordinate' && ['conflict', 'conflicts', 'conflicting', 'conflicted'].includes(keyword) && !coordinateConflictContextMatches(tokens)) return false;
  ...
  if (entry.tool === 'projscan_collision' && ['conflict', 'conflicts'].includes(keyword) && !collisionConflictContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_collision' && keyword === 'changes' && !collisionChangeContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_merge_risk' && !mergeRiskKeywordMatches(keyword, tokens)) return false;
  if (entry.tool === 'projscan_session' && ['leave', 'left', 'off'].includes(keyword) && !sessionLeaveOffContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_session' && ['away', 'asleep', 'slept', 'offline'].includes(keyword) && !sessionAwayContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_session' && keyword === 'agent' && !sessionAgentContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_workplan' && keyword === 'do' && !workplanDoContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_bug_hunt' && keyword === 'first' && ['merge', 'merged', 'merging'].some((token) => tokens.has(token))) return false;
  if (entry.tool === 'projscan_bug_hunt' && ['fastest', 'quickest', 'quick', 'smallest'].includes(keyword) && !bugHuntSpeedContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_bug_hunt' && ['small', 'low', 'lowest', 'improve', 'improvement', 'useful', 'easy', 'beginner', 'starter', 'intern', 'interns', 'task', 'tasks', 'five', 'minutes', 'today', 'win', 'wins'].includes(keyword) && !bugHuntOpportunityContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_regression_plan' && ['github', 'action', 'actions', 'workflow', 'workflows', 'pipeline', 'pipelines', 'job', 'jobs'].includes(keyword) && !regressionCiPlatformContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_regression_plan' && searchQuotedDebugTextContextMatches(tokens, hasQuotedText)) return false;
  if (entry.tool === 'projscan_regression_plan' && searchReliabilityContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_regression_plan' && searchUiInteractionContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_regression_plan' && searchIntegrationContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_regression_plan' && searchApiContractContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_regression_plan' && searchInfraArtifactContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_regression_plan' && searchDomainWorkflowContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_regression_plan' && searchCommunicationArtifactContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_regression_plan' && searchStateManagementContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_regression_plan' && searchDataAccessContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_regression_plan' && searchNavigationLayoutContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_regression_plan' && searchFrontendPageRouteContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_regression_plan' && searchStyleSystemContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_regression_plan' && verificationPlanningContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_regression_plan' && packageScriptDiscoveryContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_regression_plan' && ['port', 'ports', 'eaddrinuse', 'listen', 'address', 'permission', 'denied', 'enoent', 'eresolve', 'peer'].includes(keyword) && !regressionLocalSetupContextMatches(tokens)) return false;
  ...
  if (entry.tool === 'projscan_regression_plan' && ['test', 'tests'].includes(keyword) && (testCoverageLookupContextMatches(tokens) || coverageGapContextMatches(tokens))) return false;
  if (entry.tool === 'projscan_regression_plan' && ['run', 'rerun'].includes(keyword) && !testRunContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_regression_plan' && ['commands', 'command', 'works'].includes(keyword) && !proofCommandContextMatches(tokens) && !regressionBenchmarkContextMatches(tokens) && !regressionFlakeContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_regression_plan' && ['slow', 'slower', 'speed', 'speedup', 'faster', 'benchmark', 'benchmarks'].includes(keyword) && !regressionPerformanceContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_regression_plan' && ['reproduce', 'reproduces', 'reproducing', 'flake', 'flaky', 'flakes', 'intermittent', 'intermittently', 'nondeterministic', 'nondeterminism', 'race', 'condition', 'stabilize', 'stabilise', 'quarantine'].includes(keyword) && !regressionFlakeContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_release_train' && searchInfraArtifactContextMatches(tokens)) return false;
  if (entry.tool === 'projscan_release_train' && !releaseTrainKeywordMatches(keyword, tokens)) return false;
  return true;
}
...
function evidencePackKeywordMatches(keyword: string, tokens: Set<string>): boolean {
  ...
  if (['summarize', 'changes'].includes(keyword)) return reviewerSummaryContext || teamNarrativeContext;
  ...
  if (['ready', 'open', 'opening', 'before', 'prepare'].includes(keyword)) return prReadinessContext;
...
function reviewKeywordMatches(keyword: string, tokens: Set<string>): boolean {
  ...
  if (keyword === 'change') return tokens.has('secure') || tokens.has('security') || tokens.has('review');
  return true;
}
...
function dataflowKeywordMatches(keyword: string, tokens: Set<string>): boolean {
  ...
  if (['auth'].includes(keyword)) return tokens.has('bypass') || tokens.has('security') || tokens.has('risk') || tokens.has('risks');
  return flowContext;
}
...
function auditKeywordMatches(keyword: string, tokens: Set<string>): boolean {
  ...
  if (['audit', 'cve', 'cves', 'vulnerable', 'vulnerability', 'vulnerabilities'].includes(keyword)) return true;
...
function workspacesKeywordMatches(keyword: string, tokens: Set<string>): boolean {
  ...
  if (['map', 'list', 'owns', 'contains', 'put', 'change'].includes(keyword)) return workspaceContext || packageContext;
  return false;
}

function dependenciesKeywordMatches(keyword: string, tokens: Set<string>): boolean {
  if (dependencyCycleContextMatches(tokens) && ['dependency', 'dependencies', 'deps', 'package', 'packages'].includes(keyword)) return false;
  if (['dependencies', 'dependency', 'deps', 'packages', 'inventory', 'declared', 'supply-chain'].includes(keyword)) return true;
  ...
  if (['bundle', 'bundles', 'size', 'sizes', 'large', 'heavy', 'bloat', 'bloated', 'weight', 'footprint', 'reduce', 'slim'].includes(keyword)) return bloatContext;
  if (keyword === 'package') return licenseContext || bloatContext;
  if (!licenseContext) return false;
  if (keyword === 'source') return tokens.has('open') || tokens.has('compliance') || tokens.has('license') || tokens.has('licenses');
  ...
  if (['third', 'party'].includes(keyword)) return tokens.has('third') && tokens.has('party') && (tokens.has('notice') || tokens.has('notices') || tokens.has('license') || tokens.has('licenses') || tokens.has('compliance'));
  return true;
}
...
function couplingKeywordMatches(keyword: string, tokens: Set<string>): boolean {
  ...
  if (['circular', 'cycle', 'cycles', 'coupling', 'coupled', 'instability'].includes(keyword)) return true;
  ...
  if (['module', 'modules', 'architecture', 'boundary', 'boundaries'].includes(keyword)) return cycleSignal || couplingSignal;
...
function understandKeywordMatches(keyword: string, tokens: Set<string>): boolean {
  ...
  if (['local', 'locally', 'docker', 'compose'].includes(keyword)) return localServiceSetupCommandContextMatches(tokens) || repoSetupContextMatches(tokens);
  ...
  if (['feature', 'endpoint', 'button', 'put', 'need', 'change', 'files', 'add', 'implement', 'build', 'create', 'wire', 'route', 'component', 'page', 'screen', 'view', 'webhook', 'login', 'support', 'checkout', 'search', 'password', 'reset', 'invite', 'onboarding', 'refund', 'subscription', 'doc', 'docs', 'document', 'documentation', 'readme', 'examples', 'example', 'migration', 'migrations', 'database', 'db', 'schema', 'table', 'column', 'guide', 'update', 'updating'].includes(keyword)) {
    ...
    if (['doc', 'docs', 'document', 'documentation', 'readme', 'examples', 'example', 'guide', 'update', 'updating'].includes(keyword) && !documentationPlanningContextMatches(tokens)) return false;
    if (['migration', 'migrations', 'database', 'db'].includes(keyword) && databaseSetupCommandContextMatches(tokens)) return true;
    if (['migration', 'migrations', 'database', 'db', 'schema', 'table', 'column'].includes(keyword) && !databaseChangePlanningContextMatches(tokens)) return false;
...
function privacyCheckKeywordMatches(keyword: string, tokens: Set<string>): boolean {
  ...
  if (['env', 'values', 'code', 'source', 'local'].includes(keyword)) return privacySubjectContext || (tokens.has('read') && tokens.has('projscan'));
  if (['write', 'writes'].includes(keyword)) return privacySubjectContext;
  if (keyword === 'check') return tokens.has('privacy') || tokens.has('trust') || tokens.has('boundary');
  if (keyword === 'projscan') {
...
function databaseSetupCommandContextMatches(tokens: Set<string>): boolean {
  ...
  if (['where', 'find', 'locate', 'search', 'lookup', 'defined'].some((token) => tokens.has(token))) return false;
...
function packageScriptDiscoveryContextMatches(tokens: Set<string>): boolean {
  ...
  if ((tokens.has('npm') || tokens.has('package')) && (tokens.has('script') || tokens.has('scripts'))) return true;
...
function repoConfigContextMatches(tokens: Set<string>): boolean {
  ...
  if (tokens.has('projscan') && ['read', 'upload', 'telemetry', 'write', 'writes'].some((token) => tokens.has(token))) return false;
...
function repoOrientationContextMatches(tokens: Set<string>): boolean {
  if (['bug', 'bugs', 'fix', 'issue', 'issues', 'test', 'tests'].some((token) => tokens.has(token))) return false;
  const orientationSubject =
    ['repo', 'repository', 'codebase', 'service', 'services', 'architecture', 'entrypoint', 'entrypoints'].some((token) => tokens.has(token)) ||
...
function documentationPlanningContextMatches(tokens: Set<string>): boolean {
  if (['find', 'where', 'locate', 'search', 'lookup'].some((token) => tokens.has(token))) return false;
...
function databaseChangePlanningContextMatches(tokens: Set<string>): boolean {
  if (tokens.has('test') || tokens.has('tests')) return false;
  if (['impact', 'breaks', 'break', 'blast', 'radius', 'affect', 'used', 'usage', 'referenced', 'called', 'drop', 'delete', 'remove', 'rollback', 'revert'].some((token) => tokens.has(token))) return false;
...
function searchTestLocationContextMatches(tokens: Set<string>, hasFilePath: boolean): boolean {
  ...
  if (!['where', 'find', 'locate', 'search', 'lookup'].some((token) => tokens.has(token))) return false;
...
function testCoverageLookupContextMatches(tokens: Set<string>): boolean {
  if (['run', 'rerun', 'write', 'add', 'generate', 'plan', 'case', 'cases', 'edge', 'should'].some((token) => tokens.has(token))) return false;
...
function coverageKeywordMatches(keyword: string, tokens: Set<string>): boolean {
  if (['coverage', 'scariest', 'untested', 'uncovered', 'gap', 'gaps'].includes(keyword)) return true;
  if (['test', 'tests'].includes(keyword)) return coverageGapContextMatches(tokens);
  if (['file', 'files', 'no', 'missing', 'without'].includes(keyword)) return missingTestCoverageContextMatches(tokens);
  return false;
}

function coverageGapContextMatches(tokens: Set<string>): boolean {
  if (['coverage', 'scariest', 'untested', 'uncovered', 'gap', 'gaps'].some((token) => tokens.has(token))) return true;
...
function searchCodeLocationContextMatches(tokens: Set<string>): boolean {
  if (['run', 'rerun', 'test', 'tests', 'spec', 'specs'].some((token) => tokens.has(token))) return false;
...
function searchRouteHandlerContextMatches(tokens: Set<string>): boolean {
  if (['should', 'add', 'put', 'new', 'need', 'needs', 'change', 'implement', 'build', 'create', 'wire'].some((token) => tokens.has(token))) return false;
...
function searchEnvLookupContextMatches(tokens: Set<string>, hasEnvVar: boolean): boolean {
  ...
  if (['need', 'needs', 'required', 'requires', 'missing', 'contract', 'contracts', 'repo', 'project'].some((token) => tokens.has(token))) return false;
...
function searchTestDataContextMatches(tokens: Set<string>): boolean {
  ...
  if (['add', 'write', 'create', 'generate', 'plan', 'should', 'next', 'todo'].some((token) => tokens.has(token))) return false;
  ...
  if (!seedSubject && !fixtureSubject && !mockSubject && !factorySubject && !storySubject) return false;
...
function searchAuthorizationContextMatches(tokens: Set<string>): boolean {
  if (['add', 'create', 'implement', 'build', 'plan', 'should', 'todo', 'next'].some((token) => tokens.has(token))) return false;
  if (['fail', 'failing', 'failed', 'failure', 'failures', 'error', 'errors', 'returning', 'returns', 'runtime', 'incident', 'crash', 'crashes', 'crashing', 'outage', 'denied', '500', '502', '503', '504', '404', '403', '401'].some((token) => tokens.has(token))) return false;
  const subject = [
    'permission',
    'permissions',
    'role',
...
function searchReliabilityContextMatches(tokens: Set<string>): boolean {
  ...
  const rateLimitSubject =
    ((tokens.has('rate') || tokens.has('rates')) && (tokens.has('limit') || tokens.has('limits') || tokens.has('limiting'))) ||
    tokens.has('throttle') ||
...
function searchDataContractContextMatches(tokens: Set<string>): boolean {
  ...
  const validationSubject =
    ['validation', 'validate', 'validates', 'validator', 'schema', 'schemas', 'zod'].some((token) => tokens.has(token)) ||
  ...
  const serializationSubject =
    ['json', 'serialize', 'serializes', 'serialization', 'response'].some((token) => tokens.has(token)) ||
...
function searchDataAccessContextMatches(tokens: Set<string>): boolean {
  if (['add', 'create', 'implement', 'build', 'plan', 'should', 'todo', 'next'].some((token) => tokens.has(token))) return false;
  if (['sink', 'sinks', 'source', 'taint', 'injection', 'xss', 'vulnerability', 'security', 'sanitize', 'sanitized', 'reach', 'reaches'].some((token) => tokens.has(token))) return false;
...
function searchIntegrationContextMatches(tokens: Set<string>): boolean {
  if (['add', 'create', 'implement', 'build', 'plan', 'should', 'todo', 'next'].some((token) => tokens.has(token))) return false;
  if (tokens.has('github') && ['action', 'actions', 'workflow', 'workflows', 'job', 'jobs', 'ci'].some((token) => tokens.has(token))) return false;
  const locator = ['where', 'which', 'what', 'find', 'locate', 'search', 'lookup'].some((token) => tokens.has(token));
...
function searchInfraArtifactContextMatches(tokens: Set<string>): boolean {
  if (['add', 'create', 'implement', 'build', 'plan', 'should', 'todo', 'next'].some((token) => tokens.has(token))) return false;
  if (['fail', 'failing', 'failed', 'failure', 'failures', 'error', 'errors', 'flake', 'flaky', 'slow', 'slower'].some((token) => tokens.has(token))) return false;
  const locator = ['where', 'which', 'what', 'find', 'locate', 'search', 'lookup', 'show'].some((token) => tokens.has(token));
  const dockerSubject =
...
function searchCommunicationArtifactContextMatches(tokens: Set<string>): boolean {
  if (['add', 'create', 'implement', 'build', 'plan', 'should', 'todo', 'next'].some((token) => tokens.has(token))) return false;
  if (['leak', 'leaks', 'leaking', 'logged', 'logging', 'store', 'stores', 'retention', 'pii', 'gdpr', 'security'].some((token) => tokens.has(token))) return false;
  const locator = ['where', 'which', 'what', 'find', 'locate', 'search', 'lookup', 'show'].some((token) => tokens.has(token));
...
function searchStateManagementContextMatches(tokens: Set<string>): boolean {
  if (['add', 'create', 'implement', 'build', 'plan', 'should', 'todo', 'next'].some((token) => tokens.has(token))) return false;
  if (['leak', 'leaks', 'leaking', 'logged', 'logging', 'retention', 'pii', 'gdpr', 'security', 'secret', 'secrets', 'token', 'tokens', 'password', 'customer', 'personal'].some((token) => tokens.has(token))) return false;
  const locator = ['where', 'which', 'what', 'find', 'locate', 'search', 'lookup', 'show'].some((token) => tokens.has(token));
...
function searchNavigationLayoutContextMatches(tokens: Set<string>): boolean {
  if (['add', 'create', 'implement', 'build', 'plan', 'should', 'todo'].some((token) => tokens.has(token))) return false;
  const locator = ['where', 'which', 'what', 'find', 'locate', 'search', 'lookup', 'show'].some((token) => tokens.has(token));
  const navSubject =
  ...
  const titleSubject =
    (tokens.has('page') && tokens.has('title')) ||
    tokens.has('metadata') ||
    tokens.has('meta');
  const layoutSubject =
    tokens.has('layout') &&
...
function searchFrontendPageRouteContextMatches(tokens: Set<string>): boolean {
  if (['add', 'create', 'implement', 'build', 'plan', 'should', 'todo'].some((token) => tokens.has(token))) return false;
  if (['why', 'returning', 'returns', 'failing', 'failed', 'failure', 'failures', 'production', 'prod', 'down', 'outage', 'incident', 'runtime', 'crash', 'crashes', 'crashing'].some((token) => tokens.has(token))) return false;
  const locator = ['where', 'which', 'what', 'find', 'locate', 'search', 'lookup', 'show'].some((token) => tokens.has(token));
...
function searchStyleSystemContextMatches(tokens: Set<string>): boolean {
  if (['add', 'create', 'implement', 'build', 'plan', 'should', 'todo'].some((token) => tokens.has(token))) return false;
  if (['why', 'failing', 'failed', 'failure', 'failures', 'broken', 'error', 'errors', 'runtime', 'production', 'prod', 'outage', 'incident'].some((token) => tokens.has(token))) return false;
  const locator = ['where', 'which', 'what', 'find', 'locate', 'search', 'lookup', 'show'].some((token) => tokens.has(token));
...
function searchConfigLookupContextMatches(tokens: Set<string>): boolean {
  ...
  if (['need', 'needs', 'required', 'requires', 'missing', 'contract', 'contracts', 'env', 'environment', 'vars', 'variables'].some((token) => tokens.has(token))) return false;
...
function searchToolingConfigContextMatches(tokens: Set<string>): boolean {
  if (['add', 'create', 'implement', 'build', 'plan', 'should', 'todo', 'update', 'upgrade', 'bump', 'remove', 'drop', 'uninstall'].some((token) => tokens.has(token))) return false;
  if (['why', 'failing', 'failed', 'failure', 'failures', 'broken', 'error', 'errors', 'runtime', 'production', 'prod', 'outage', 'incident'].some((token) => tokens.has(token))) return false;
...
function searchMigrationLookupContextMatches(tokens: Set<string>): boolean {
  ...
  if (['should', 'put', 'add', 'change', 'need', 'needs', 'plan', 'deploy', 'zero', 'downtime'].some((token) => tokens.has(token))) return false;
  if (['impact', 'breaks', 'break', 'breaking', 'blast', 'radius', 'affect', 'drop', 'delete', 'remove', 'rollback', 'revert'].some((token) => tokens.has(token))) return false;
...
function searchGeneratedContextMatches(tokens: Set<string>): boolean {
  ...
  if (['write', 'create', 'generate', 'regenerate', 'build', 'emit'].some((token) => tokens.has(token))) return false;
...
function searchOwnershipContextMatches(tokens: Set<string>, hasFilePath: boolean): boolean {
  ...
  if (['pr', 'pull', 'request', 'review', 'reviewer', 'reviewers'].some((token) => tokens.has(token))) return false;
...
function fileTestContextMatches(tokens: Set<string>): boolean {
  if (['coverage', 'covered', 'uncovered', 'add', 'write'].some((token) => tokens.has(token))) return true;
...
function impactRollbackContextMatches(keyword: string, tokens: Set<string>): boolean {
  ...
  if (keyword === 'recover') return tokens.has('bad') || tokens.has('deploy') || tokens.has('deployment');
  return false;
}

function doctorCleanupDeleteContextMatches(tokens: Set<string>, hasFilePath: boolean, hasPackageRemoval: boolean): boolean {
...
function hotspotWhereContextMatches(tokens: Set<string>, hasFilePath: boolean): boolean {
  return (
    ['focus', 'risky', 'riskiest', 'dangerous', 'hotspot', 'hotspots', 'churn', 'complexity', 'complex', 'refactor', 'refactoring', 'simplify', 'simplification', 'tech', 'debt', 'duplicate', 'duplicated', 'duplication', 'over', 'engineered', 'performance', 'perf', 'bottleneck', 'bottlenecks', 'optimize', 'optimise', 'faster', 'slow'].some((token) => tokens.has(token)) ||
...
function prDiffKeywordMatches(keyword: string, tokens: Set<string>): boolean {
  ...
  if (['compare', 'stale', 'branched', 'behind', 'ahead', 'sync', 'synced'].includes(keyword)) return branchSyncDiffContextMatches(tokens);
  if (keyword === 'change') {
...
function branchSyncDiffContextMatches(tokens: Set<string>): boolean {
  if (['conflict', 'conflicts', 'resolve', 'resolving', 'rebase', 'rebasing'].some((token) => tokens.has(token))) return false;
...
function mergeRiskKeywordMatches(keyword: string, tokens: Set<string>): boolean {
  if (keyword === 'first') return ['merge', 'merged', 'merging', 'integrate', 'integration'].some((token) => tokens.has(token));
  if (keyword === 'branch') return ['first', 'order', 'sequence', 'integrate', 'integration'].some((token) => tokens.has(token));
  return true;
}
...
function regressionFlakeContextMatches(tokens: Set<string>): boolean {
  ...
  if (flakeSignal) return verificationSubject || reproduceSignal || stabilizationSignal || raceSignal;
...
function keywordWeight(entry: RouteEntry, keyword: string): number {
  if (entry.tool === 'projscan_privacy_check') {
    if (['privacy', 'trust', 'boundary', 'env', 'upload', 'telemetry', 'network', 'offline', 'leave', 'machine'].includes(keyword)) return 3;
    if (['read', 'code', 'source', 'values', 'contact', 'contacted', 'write', 'writes', 'projscan'].includes(keyword)) return 2;
  }
  ...
  if (entry.tool === 'projscan_file' && ['review', 'reviewer', 'reviewers'].includes(keyword)) return 5;
  if (entry.tool === 'projscan_file' && ['file', 'explain', 'inspect', 'owns', 'risk', 'risks', 'risky', 'dangerous'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_file' && ['last', 'touched', 'touch', 'changed', 'recently', 'history', 'author', 'authors', 'blame'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_file' && ['add', 'write'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_file' && ['coverage', 'covered', 'uncovered', 'test', 'tests'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_impact' && ['delete', 'remove'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_impact' && ['drop', 'schema', 'table', 'column', 'database', 'db', 'migration', 'migrations'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_impact' && ['revert', 'rollback', 'undo', 'backout', 'back', 'out', 'recover'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_impact' && ['depends', 'used', 'usage', 'referenced', 'called'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_impact' && ['api', 'apis', 'endpoint', 'endpoints', 'client', 'clients', 'contract', 'contracts', 'deprecate', 'deprecates', 'deprecated', 'deprecation', 'compatibility', 'compatible', 'version', 'versions'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_impact' && ['change', 'changes', 'changing'].includes(keyword)) return 3;
  if (entry.tool === 'projscan_explain_issue' && keyword === 'explain') return 2;
  if (entry.tool === 'projscan_semantic_graph' && ['import', 'imports', 'importers', 'exports', 'defined', 'definition', 'uses', 'depend', 'depends', 'installed'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_coupling') {
    if (['circular', 'cycle', 'cycles'].includes(keyword)) return 3;
    if (['coupling', 'coupled', 'tightly', 'module', 'modules', 'dependency', 'dependencies', 'import', 'imports', 'fan', 'instability', 'architecture', 'boundary', 'boundaries'].includes(keyword)) return 2;
  }
  if (entry.tool === 'projscan_coverage' && ['coverage', 'untested', 'uncovered', 'scariest', 'gap', 'gaps', 'test', 'tests', 'file', 'files', 'no', 'missing', 'without'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_dependencies' && ['dependencies', 'dependency', 'deps', 'package', 'packages', 'inventory', 'declared'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_dependencies' && ['bundle', 'bundles', 'size', 'sizes', 'large', 'heavy', 'bloat', 'bloated', 'weight', 'footprint', 'reduce', 'slim'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_dependencies' && ['license', 'licenses', 'gpl', 'copyleft', 'notice', 'notices', 'third', 'party', 'open', 'source', 'compliance'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_workspaces' && ['workspace', 'workspaces', 'monorepo', 'package', 'packages', 'map', 'list', 'owns', 'contains', 'put', 'change'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_upgrade' && ['upgrade', 'bump', 'update', 'remove', 'drop', 'uninstall', 'package'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_audit') {
    if (['audit', 'cve', 'cves', 'vulnerable', 'vulnerability', 'vulnerabilities', 'security', 'secure', 'safe'].includes(keyword)) return 2;
  ...
  if (entry.tool === 'projscan_search' && ['welcome', 'template', 'templates', 'copy', 'push', 'sms', 'verification', 'receipt', 'invoice', 'pdf'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_search' && ['password', 'reset', 'invite', 'invites', 'onboarding', 'flow', 'flows', 'csv', 'export', 'exports', 'audit', 'entries', 'refund', 'handling', 'payments', 'subscription', 'renewal', 'users', 'creates'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_search' && ['prisma', 'drizzle', 'typeorm', 'sequelize', 'sql', 'model', 'models', 'entity', 'entities', 'repository', 'repositories', 'dao', 'daos', 'saves', 'save', 'persist', 'persists', 'orders'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_search' && ['docker', 'dockerfile', 'compose', 'containerfile', 'kubernetes', 'k8s', 'manifest', 'manifests', 'helm', 'chart', 'charts', 'terraform', 'tf', 'module', 'modules', 'cloudformation', 'cdk', 'pulumi', 'vercel', 'netlify', 'railway', 'fly', 'workflow', 'workflows', 'deploy', 'deploys', 'deployment', 'staging', 'production'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_search' && ['openapi', 'swagger', 'trpc', 'grpc', 'protobuf', 'proto', 'protos', 'router', 'routers', 'resolver', 'resolvers'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_search' && ['call', 'calls', 'called', 'client', 'clients', 'sdk', 'sdks', 'integration', 'integrations', 'stripe', 'sendgrid', 's3', 'github', 'graphql', 'websocket', 'websockets', 'socket', 'sockets', 'connection', 'connections', 'rest', 'http', 'fetch', 'axios', 'external', 'service', 'services', 'upload', 'uploads', 'uploaded', 'sent', 'opened'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_search' && ['store', 'stores', 'stored', 'redux', 'slice', 'slices', 'selector', 'selectors', 'zustand', 'jotai', 'recoil', 'context', 'provider', 'providers', 'supplies', 'supplied', 'provides', 'provided', 'hook', 'hooks', 'react', 'query', 'queries', 'mutation', 'mutations', 'fetches', 'fetched', 'invoices'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_search' && ['sidebar', 'nav', 'navigation', 'menu', 'item', 'items', 'breadcrumb', 'breadcrumbs', 'layout', 'next', 'js', 'title', 'metadata', 'meta', 'billing', 'settings', 'checkout'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_search' && ['page', 'segment', 'segments', 'not', 'found', '404', 'render', 'renders', 'rendered', 'handled', 'route', 'routes', 'dashboard'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_search' && ['design', 'token', 'tokens', 'tailwind', 'theme', 'themes', 'css', 'global', 'imported', 'style', 'styles', 'styled', 'class', 'classes', 'dark', 'mode', 'breakpoint', 'breakpoints', 'color', 'colors'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_search' && ['form', 'forms', 'submit', 'submits', 'submitted', 'loading', 'state', 'empty', 'results', 'boundary', 'toast', 'notification', 'notifications', 'success', 'keyboard', 'shortcut', 'shortcuts', 'command', 'palette', 'action', 'actions', 'modal', 'opened', 'component', 'page', 'i18n', 'translation', 'translations', 'aria', 'label', 'button', 'buttons', 'focus', 'trap'].includes(keyword)) return 2;
  ...
  if (entry.tool === 'projscan_search' && ['tsconfig', 'typescript', 'vite', 'vitest', 'jest', 'babel', 'webpack', 'pnpm', 'yarn', 'npm', 'package', 'manager', 'workspace', 'workspaces', 'lockfile', 'lockfiles', 'path', 'paths'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_regression_plan' && ['port', 'ports', 'eaddrinuse', 'listen', 'address', 'permission', 'denied', 'enoent', 'eresolve', 'peer'].includes(keyword)) return 3;
  ...
  if (entry.tool === 'projscan_agent_brief' && ['brief', 'handoff', 'agent'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_session' && ['session', 'touched', 'touch', 'resume', 'leave', 'left', 'off', 'agent', 'asleep', 'slept', 'away', 'offline', 'changed', 'events', 'history'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_quality_scorecard' && ['quality', 'scorecard', 'risk', 'risks', 'risky', 'picture'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_hotspots' && ['files', 'file', 'touch', 'complexity', 'complex', 'refactor', 'refactoring', 'simplify', 'simplification', 'tech', 'debt', 'duplicate', 'duplicated', 'duplication', 'over', 'engineered', 'performance', 'perf', 'bottleneck', 'bottlenecks', 'optimize', 'optimise', 'faster', 'slow'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_coordinate' && ['who', 'else', 'working', 'editing', 'coordinate', 'coordination', 'status', 'readiness', 'parallel', 'agents', 'agent', 'collide', 'colliding', 'swarm', 'conflict', 'conflicts', 'conflicting', 'conflicted', 'active'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_preflight' && keyword === 'ready') return 2;
  if (entry.tool === 'projscan_preflight' && ['block', 'blocked', 'blocker', 'blockers', 'blocking'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_preflight' && ['risk', 'risks'].includes(keyword)) return 2;
  if (entry.tool === 'projscan_preflight' && ['rebase', 'rebasing', 'conflict', 'conflicts', 'resolve', 'resolving', 'wrong', 'stuck'].includes(keyword)) return 2;
  ...
  if (entry.tool === 'projscan_evidence_pack') {
    ...
    if (['evidence', 'proof', 'approval', 'approve', 'comment', 'summarize', 'changes', 'description', 'draft', 'say', 'checklist', 'tell', 'team', 'change', 'share', 'reviewer', 'reviewers', 'summary', 'packet', 'paste', 'who', 'review', 'ready', 'open', 'opening', 'before', 'prepare', 'owner', 'owners', 'owns', 'routing'].includes(keyword)) return 2;
  }
  ...
  if (entry.tool === 'projscan_pr_diff') {
    ...
    if (['commit', 'message', 'summarize', 'summary', 'diff', 'changed', 'changes', 'change', 'compare', 'stale', 'branched', 'behind', 'ahead', 'sync', 'synced', 'large', 'big', 'size', 'sizes', 'exports', 'imports', 'calls', 'callers'].includes(keyword)) return 2;
  }
  ...
  if (entry.tool === 'projscan_bug_hunt') {
    if (['bug', 'bugs', 'hunt', 'defect', 'broken', 'first', 'fastest', 'quickest', 'quick', 'smallest', 'small', 'low', 'lowest', 'improve', 'improvement', 'useful', 'easy', 'beginner', 'starter', 'intern', 'interns', 'task', 'tasks', 'five', 'minutes', 'today', 'win', 'wins'].includes(keyword)) return 2;
