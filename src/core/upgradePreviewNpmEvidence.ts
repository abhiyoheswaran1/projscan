...
import { compare as compareSemver, parse as parseSemver } from '../utils/semver.js';
...
export interface NpmChangelogEvidence {
...
export async function readDeclaredVersion(rootPath: string, name: string): Promise<string | null> {
...
export async function readInstalledVersion(rootPath: string, name: string): Promise<string | null> {
...
export async function readNpmChangelogEvidence(
  rootPath: string,
  name: string,
  declaredVersion: string | null,
  installedVersion: string | null,
): Promise<NpmChangelogEvidence> {
  try {
    const changelog = await readChangelog(rootPath, name);
    if (!changelog) return emptyChangelogEvidence();

    const slice = sliceBetween(changelog, declaredVersion, installedVersion);
    return {
      breakingMarkers: detectBreakingMarkers(slice),
      changelogExcerpt: truncate(slice, 4000),
    };
  } catch {
    return emptyChangelogEvidence();
  }
}

function emptyChangelogEvidence(): NpmChangelogEvidence {
  return { breakingMarkers: [] };
}
...
/**
 * Extract the CHANGELOG section strictly between two versions. If headings
 * cannot be located, return the top 100 lines.
 */
