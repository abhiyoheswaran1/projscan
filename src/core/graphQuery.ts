...
export function runGraphQuery(graph: CodeGraph, input: GraphQueryInput): Record<string, unknown> {
  ...
  switch (direction) {
    case 'imports': {
      if (!file) throw new Error('direction=imports requires a `file` argument (repo-relative path, e.g. "src/auth.ts").');
      return { file, imports: importsOf(graph, file).slice(0, limit) };
    }
    case 'exports': {
      if (!file) throw new Error('direction=exports requires a `file` argument (repo-relative path).');
      return { file, exports: exportsOf(graph, file).slice(0, limit) };
    }
    case 'importers': {
      if (!file) throw new Error('direction=importers requires a `file` argument (repo-relative path).');
      return { file, importers: filesImportingFile(graph, file).slice(0, limit) };
    }
    case 'symbol_defs': {
      if (!symbol) throw new Error('direction=symbol_defs requires a `symbol` argument (the exported name, e.g. "authenticate").');
      return { symbol, definedIn: filesDefiningSymbol(graph, symbol).slice(0, limit) };
    }
    case 'package_importers': {
      const pkg = symbol ?? file;
      if (!pkg) throw new Error('direction=package_importers requires a `symbol` (or `file`) arg (the npm package name, e.g. "chalk").');
