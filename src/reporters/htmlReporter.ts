...
export function reportAnalysisHtml(
  ...
  const languageRows = Object.values(report.languages.languages)
...
<td class="muted">${lang.extensions.map((ext) => `<code>${escapeHtml(ext)}</code>`).join(' ')}</td>
  ...
  const issueRows =
    report.issues.length === 0
      ? '<tr><td colspan="4" class="muted">No issues detected.</td></tr>'
      : report.issues
          .slice(0, 100)
          .map(
            (issue) => `<tr>
...
<td>${issue.locations?.map((loc) => `<code>${escapeHtml(loc.file)}${loc.line ? `:${loc.line}` : ''}</code>`).join('<br>') ?? '<span class="muted">—</span>'}</td>
  ...
  const fileRows = report.scan.files
    .slice(0, 100)
    .map(
      (file) => `<tr>
...
<p><strong>Dependencies:</strong> ${escapeHtml(dependencySummary)}</p>
</div>
<h2>Languages</h2>
<table>
<thead><tr><th>Language</th><th class="right">Files</th><th class="right">Share</th><th>Extensions</th></tr></thead>
<tbody>${languageRows || '<tr><td colspan="4" class="muted">No source files detected.</td></tr>'}</tbody>
</table>
...
function reportControlsHtml(reportControls: ReportControlsMetadata | undefined): string {
