...
const DEPRECATED_WARNING = new Set(['nose', 'simplejson', 'pycrypto', 'mysql-python']);

const DEPRECATED_INFO = new Set(['python-dateutil']);

const HEAVY_INFO = new Set(['pandas', 'numpy', 'torch', 'tensorflow']);

const DEPRECATION_REASONS: Record<string, string> = {
  nose: 'nose is retired. Use pytest instead.',
  simplejson:
    'simplejson is no longer needed; the stdlib json module is equivalent for most use cases.',
...
const HEAVY_REASONS: Record<string, string> = {
  pandas:
    'pandas is a heavy dependency (~30MB). Reach for it only if you actually need dataframes.',
  ...
  tensorflow:
    'tensorflow is a very heavy dependency. Consider tensorflow-cpu for inference-only use.',
};
...
export async function check(rootPath: string, files: FileEntry[]): Promise<Issue[]> {
  ...
  for (const dep of info.declared) {
    ...
    if (dep.source.endsWith('.txt') && (dep.versionSpec === '' || dep.versionSpec === '*')) {
      seen.add(id);
      issues.push({
        id,
