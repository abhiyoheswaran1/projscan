  ...
  inputSchema: {
    ...
    properties: {
      action: {
        ...
        description:
          'Default "list". "add" records a claim; "release" drops one; "prune" removes expired leases.',
      },
      target: {
        type: 'string',
      ...
      ttl_seconds: {
        type: 'number',
        description:
          '"add" — lease duration in seconds; the claim expires after it. Omit for a permanent claim.',
      },
    ...
    switch (action) {
      ...
      case 'release': {
        const selector = {
          ...
          target:
            typeof args.target === 'string' && args.target.length > 0 ? args.target : undefined,
