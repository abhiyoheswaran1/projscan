...
export function registerFixSuggest(): void {
  program
    ...
    .description(
      'Get a structured fix-action prompt for an issue (rule-driven; no LLM inside projscan)',
    )
    ...
    .option(
      '--severity <level>',
      'severity for synthesized requests (info | warning | error)',
      'warning',
    )
    .action(
      async (
        issueId: string | undefined,
        cmdOpts: { file?: string; rule?: string; severity?: string },
      ) => {
        setupLogLevel();
        maybeCompactBanner();
        ...
        try {
          ...
          if (issueId) {
            ...
            const sev =
              cmdOpts.severity === 'info' ||
              cmdOpts.severity === 'warning' ||
              cmdOpts.severity === 'error'
                ? cmdOpts.severity
                : 'warning';
            ...
            result = {
              matched: false,
              reason: 'Provide either an <issue_id> argument or both --file and --rule.',
            };
          }
