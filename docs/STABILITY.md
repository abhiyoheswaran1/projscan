...
- **Output formats**: `console`, `json`, `markdown`, `sarif`, `html`. The `--format` flag will continue to accept these names. Individual commands may support only a subset; unsupported combinations fail with a clear diagnostic rather than falling back to a different renderer. Per-format guarantees:
  - **JSON**: existing report-level `schemaVersion` values remain stable for their report families. The semantic graph contract uses `schemaVersion: 3`; graph-evidence summaries are additive optional fields. Data keys (`issues`, `hotspots`, `coverage`, `nodes`, `edges`, etc.) are stable. New optional fields may be added to objects without a major bump; existing field names and types will not change.
    `projscan prove` reports include `verifiedWorkflow` as an additive summary for contract, run, record, and receipt modes. Proof contracts may include additive `proofRequirements`, and Proof Receipts may include additive `proofSufficiency` rows for per-surface proof status. Proof Receipts may also include additive `proofReplay` fields with replay status, timeline events, `changedAfterProof`, replay command, and receipt fingerprint. `projscan passport` reports include stable boundary, receipt, reviewer, next-command, artifact, and proof summary sections. `projscan guard` reports include stable status, drift, proof, and reviewer-action sections. Team Proof Recipe config under `proofRecipes` may add `teamProofRecipes`, `requiredReviewers`, `recipeGaps`, and `recipeDrift` fields to Proof Contracts, Proof Receipts, passports, and evidence-pack PR-comment summaries.
