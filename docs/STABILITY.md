...
- **Command names**: `agent-brief`, `analyze`, `apply-fix`, `assess`, `audit`, `badge`, `bug-hunt`, `ci`, `collisions`, `claim`, `coordinate`, `coupling`, `coverage`, `dataflow`, `dependencies`, `diagram`, `diff`, `doctor`, `dogfood`, `evidence-pack`, `explain-issue`, `feedback`, `file`, `first-run`, `fix`, `fix-suggest`, `guard`, `handoff`, `help`, `hotspots`, `impact`, `init`, `install-hook`, `mcp`, `memory`, `merge-risk`, `mission-proof`, `outdated`, `passport`, `plugin`, `preflight`, `pr-diff`, `privacy-check`, `prove`, `proof-broker`, `quality-scorecard`, `recipes`, `regression-plan`, `release-train`, `review`, `review-gate`, `route`, `search`, `semantic-graph`, `session`, `simulate`, `start`, `structure`, `taint`, `telemetry`, `trial`, `understand`, `upgrade`, `watch`, `workplan`, `workspace`, `workspaces`. New commands may be added; existing names will not be renamed or removed.
- **Documented flags** on those commands: `--format`, `--config`, `--changed-only`, `--base-ref`, `--package`, `--report-policy`, `--report-scope`, `--redact-paths`, `--limit`, `--cycles-only`, `--high-fan-in`, `--high-fan-out`, `--file`, `--mode`, `--view`, `--intent`, `--plan`, `--contract`, `--save-contract`, `--record-command`, `--run`, `--run-timeout-ms`, `--exit-code`, `--duration-ms`, `--ledger`, `--feedback`, `--baseline`, `--max-files`, `--max-items`, `--semantic`, `--scope`, `--min-score`, `--save-baseline`, `--against`, `--timeout`, `--aggregate`, `--output`, `--output-passport`, `--pr-comment`, `--ci`, `--fail-on-block`, `--fail-on-needs-proof`, `--task-id`, `--emit-baseframe`, `--watch`, `--fail-on-drift`, `--verbose`, `--quiet`. Documented at `projscan <cmd> --help` or in `docs/GUIDE.md`.
- **Exit codes**: `0` = success / pass, `1` = found issues, failed gate, or command-specific validation failure, `2` = invalid usage reported by the shared CLI parser. We will not flip an existing code's meaning.
- **Output formats**: `console`, `json`, `markdown`, `sarif`, `html`. The `--format` flag will continue to accept these names. Individual commands may support only a subset; unsupported combinations fail with a clear diagnostic rather than falling back to a different renderer. Per-format guarantees:
  - **JSON**: existing report-level `schemaVersion` values remain stable for their report families. The semantic graph contract uses `schemaVersion: 3`; graph-evidence summaries are additive optional fields. Data keys (`issues`, `hotspots`, `coverage`, `nodes`, `edges`, etc.) are stable. New optional fields may be added to objects without a major bump; existing field names and types will not change.
    `projscan prove` reports include `verifiedWorkflow` as an additive summary for contract, run, record, and receipt modes. Proof contracts may include additive `proofRequirements`, and Proof Receipts may include additive `proofSufficiency` rows for per-surface proof status. Proof Receipts may also include additive `proofReplay` fields with replay status, timeline events, `changedAfterProof`, replay command, and receipt fingerprint. `projscan passport` reports include stable boundary, receipt, reviewer, next-command, artifact, and proof summary sections. `projscan proof-broker` reports include stable required-proof, required-reviewer, risky-changed-file, gap, next-command, PR Passport, and embedded passport sections. `projscan review-gate` reports include stable status, allow-review decision, proof-debt, recontract, required-reviewer, next-command, PR-comment, artifact, and embedded Proof Broker sections. `projscan guard` reports include stable status, drift, proof, and reviewer-action sections. Team Proof Recipe config under `proofRecipes` may add `teamProofRecipes`, `requiredReviewers`, `recipeGaps`, and `recipeDrift` fields to Proof Contracts, Proof Receipts, passports, Proof Broker reports, Review Gate reports, and evidence-pack PR-comment summaries.
...
- **Tool names** (via `tools/list`): `projscan_analyze`, `projscan_doctor`, `projscan_hotspots`, `projscan_file`, `projscan_structure`, `projscan_dependencies`, `projscan_outdated`, `projscan_audit`, `projscan_upgrade`, `projscan_coverage`, `projscan_semantic_graph`, `projscan_coupling`, `projscan_workspaces`, `projscan_pr_diff`, `projscan_review`, `projscan_fix_suggest`, `projscan_explain_issue`, `projscan_impact`, `projscan_search`, `projscan_session`, `projscan_memory`, `projscan_workspace_graph`, `projscan_apply_fix`, `projscan_taint`, `projscan_dataflow`, `projscan_cost_summary`, `projscan_review_watch`, `projscan_plugin`, `projscan_preflight`, `projscan_workplan`, `projscan_release_train`, `projscan_bug_hunt`, `projscan_evidence_pack`, `projscan_regression_plan`, `projscan_agent_brief`, `projscan_quality_scorecard`, `projscan_assess`, `projscan_simulate`, `projscan_prove`, `projscan_passport`, `projscan_proof_broker`, `projscan_review_gate`, `projscan_adoption`, `projscan_start`, `projscan_understand`, `projscan_collision`, `projscan_claim`, `projscan_merge_risk`, `projscan_route`, `projscan_coordinate`, `projscan_coordinate_watch`. New tools may be added without a major bump; existing names will not be renamed or removed.
...
- **Resource URIs**: `projscan://health`, `projscan://hotspots`, `projscan://structure`, `projscan://session/summary`, `projscan://handoff`, `projscan://risk-now`. Resource payloads are JSON. New optional fields may appear; existing URI names will not be renamed or repurposed in 4.x.

### Configuration

- **`.projscanrc`** schema: `ignore`, `disableRules`, `severityOverrides`, `hotspots.limit`, `hotspots.since`, `reportPolicies.<name>.reportScope`, `reportPolicies.<name>.redactPaths`, and `proofRecipes[]` with `id`, `matches`, `requiredCommands`, optional `requiredReviewers`, optional `forbiddenFiles`, optional `riskSurface`, and optional `reason`. New keys may be added; existing keys will not change name or type.
- **Environment variables** consulted: `PROJSCAN_PLUGINS_PREVIEW=1` enables local plugin execution. `PROJSCAN_TELEMETRY_HOME`, `PROJSCAN_TELEMETRY_ENDPOINT`, `PROJSCAN_TELEMETRY_DISABLED`, and `PROJSCAN_TELEMETRY_NO_NETWORK` control the explicit opt-in telemetry storage, endpoint, hard-disable, and queue-only modes. Existing names remain stable in 4.x.
...
optional manifest fields may be added in 4.x; existing required fields keep
