...
- This update should move the site from the 4.0.0 consolidation story to the 4.3.0 Mission Outcome Loop story. Keep the 4.0 migration section as historical context.

```text
Update the projscan website for projscan 4.3.0.
...
Release headline:
4.3.0: Mission Outcome Loop

Primary story:
projscan now closes the loop after an agent runs a Mission Control bundle. A developer or agent can save a mission, run local proof, resume from the saved proof state, see what changed and what remains, and decide whether the work is ready for version review.
...
- Version: `4.0.0` -> `4.3.0`.
- Navigation/changelog label: `Changelog 4.0.0` -> `Changelog 4.3.0`.
- Release notes heading: `Shipped in 4.0.0.` -> `Shipped in 4.3.0.`
...
New release-note bullets for 4.3.0:
...
- Release trust: mention there are no MCP tool removals in 4.3.0. This release adds CLI and start-report surfaces around Mission Control proof.
...
- Keep the 4.0 migration section in place, and state that 4.3.0 is additive.

Screenshots/media:
- Use the README media after the `v4.3.0` tag is live:
  - `https://raw.githubusercontent.com/abhiyoheswaran1/projscan/v4.3.0/docs/projscan-mission-control.png`
  - `https://raw.githubusercontent.com/abhiyoheswaran1/projscan/v4.3.0/docs/projscan-proof-router.png`
...
Ask projscan what you are trying to do, then keep the proof with the work. In 4.3.0, `projscan start --intent "<goal>"` builds a mission, `--save-mission` writes the runnable bundle, and `projscan start --mission .projscan/mission` resumes from the latest local proof state. `projscan mission-proof` summarizes pass/fail evidence, reviewer decisions, reruns, failed gates, and optional local baseline savings. It stays local-first: no source upload, no API key, no hidden telemetry.

Verification before publishing:
- Confirm every visible version/changelog label reads 4.3.0.
...
- Confirm the screenshots load from the `v4.3.0` raw GitHub URLs after the tag is pushed.
...
- Changelog source: `CHANGELOG.md` entry for `4.3.0`
