...
- This update should move the site from the 4.0.0 consolidation story to the 4.6.0 Agent Coordination And Routing Hardening story. Keep the 4.0 migration section as historical context.

```text
Update the projscan website for projscan 4.6.0.
...
Release headline:
4.6.0: Agent Coordination And Routing Hardening

Primary story:
projscan now hardens the agent workflow loop with broader framework dataflow precision, coordination evidence for multi-agent work, exported code graph result types, and a smaller routing test surface while preserving local-first review evidence.
...
- Keep 4.0 migration content, but move it below the new 4.6 release story.

Stale live-site claims to update:
- Version: `4.0.0`, `4.3.1`, or `4.4.0` -> `4.6.0`.
- Navigation/changelog label: `Changelog 4.0.0`, `Changelog 4.3.1`, or `Changelog 4.4.0` -> `Changelog 4.6.0`.
- Release notes heading: `Shipped in 4.0.0.`, `Shipped in 4.3.1.`, or `Shipped in 4.4.0.` -> `Shipped in 4.6.0.`
...
New release-note bullets for 4.6.0:
- Framework dataflow coverage: Next `nextUrl`, Hono URL reads, Express URL reads, Koa URL reads, and Fastify URL reads remain framework-gated and fixture-backed.
- Coordination evidence: agent hints expose the local command evidence behind coordination, collision, and handoff decisions.
...
- Maintainability cleanup: `tests/core/start.test.ts` is reduced to default/workspace/ownership/coupling coverage with focused companion files, and helpers are extracted across intent routing, code graph, release evidence, upgrade preview, CLI, and MCP surfaces.
...
- Release trust: broad release verification and AgentLoopKit evidence are tied to the same task contract that drove the release.
...
- Release trust: mention there are no MCP tool removals in 4.6.0. This release adds framework request-source coverage, coordination evidence, public code graph result types, routing hardening, and release-review evidence around Mission Control.
...
- Keep the 4.0 migration section in place, and state that 4.6.0 is additive.

Screenshots/media:
- Use the README media after the `v4.6.0` tag is live:
  - `https://raw.githubusercontent.com/abhiyoheswaran1/projscan/v4.6.0/docs/projscan-mission-control.png`
  - `https://raw.githubusercontent.com/abhiyoheswaran1/projscan/v4.6.0/docs/projscan-proof-router.png`
  - `https://raw.githubusercontent.com/abhiyoheswaran1/projscan/v4.6.0/docs/projscan-mission-control.gif`
  - `https://raw.githubusercontent.com/abhiyoheswaran1/projscan/v4.6.0/docs/projscan-mission-proof.gif`
...
Ask projscan what you are trying to do, then keep the proof with the work. In 4.6.0, `projscan start --intent "<goal>"` adds stronger coordination evidence for multi-agent workflows, broader framework-aware dataflow checks, exported code graph result types, and a smaller routing test surface behind the same local AgentLoopKit proof loop. It stays local-first: no source upload, no API key, no hidden telemetry.

Verification before publishing:
- Confirm every visible version/changelog label reads 4.6.0.
...
- Confirm the screenshots load from the `v4.6.0` raw GitHub URLs after the tag is pushed.
...
- Changelog source: `CHANGELOG.md` entry for `4.6.0`
