...
4. Python upgrade intelligence
   - `projscan upgrade` and MCP `projscan_upgrade` now support offline Python
     ...
     `requirements*.txt`, Poetry lockfiles, and pinned root requirements.
...
- Supported current-version evidence is intentionally narrow: Poetry package
  blocks and pinned root requirements.
